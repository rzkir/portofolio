...
export default function YoutubeLayout() {
    ...
    const [youtubeContent, setYoutubeContent] = useState<youtube[]>([]);
    const [isEditing, setIsEditing] = useState(false);
    const [formData, setFormData] = useState<youtube>({
        _id: '',
        title: '',
        href: '',
        description: '',
    ...
    useEffect(() => {
        const fetchData = async () => {
            ...
            try {
                ...
                const youtubeData = await youtubeResponse.json() as youtube[];
    ...
    const handleEdit = (content: youtube) => {
        setFormData({
            _id: content._id,
            title: content.title,
            href: content.href,
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            const url = isEditing ? `/api/youtube?id=${formData._id}` : '/api/youtube';
            ...
            if (isEditing) {
                ...
                setYoutubeContent(prev => [...prev, data]);
            }
    ...
    const handleDelete = async () => {
        ...
        setIsDeleting(true);
        try {
            const response = await fetch(`/api/youtube?id=${deleteContentId}`, {
                method: 'DELETE',
            });
            ...
            setYoutubeContent(prev => prev.filter(content => content._id !== deleteContentId));
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 md:gap-6 mt-6">
                {isLoading ? (
                    ...
                    <div className="col-span-full flex flex-col items-center justify-center p-8 text-center border rounded-lg bg-muted/50">
                        ...
                        <p className="text-muted-foreground mb-4">There are no YouTube contents available at the moment.</p>
                        <Button
                            variant="default"
                            onClick={() => setIsOpen(true)}
